...
class PriceHelper
{
    ...
    public static function checkOrderProductReivew($order_id, $user_id, $product_id)
    {
        $returnArray = [];
        $returnArray['status'] = 'no';
        $reviewsData = Rating::where('user_id', $user_id)->where('product_id', $product_id)->where('order_id', $order_id)->first();
        if (!empty($reviewsData)) {
            $returnArray['status'] = 'yes';
            $returnArray['reviewData'] = $reviewsData;
        }

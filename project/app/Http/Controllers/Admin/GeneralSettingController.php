...
class GeneralSettingController extends AdminBaseController
{
    ...
    [
        ...
        'deal_banner'       => 'mimes:jpeg,jpg,png,svg',
        'gift_banner'       => 'mimes:jpeg,jpg,png,svg',
    ];
    ...
    public function generalupdate(Request $request)
    {
        ...
        $data = Generalsetting::findOrFail(1);
            ...
            if ($file = $request->file('footer_logo'))
            {
             ...
             if ($file = $request->file('deal_banner'))
            {
                $name = \PriceHelper::ImageCreateName($file);
                $data->upload($name,$file,$data->deal_banner);
                $input['deal_banner'] = $name;
            }
             if ($file = $request->file('gift_banner'))
            {
                $name = \PriceHelper::ImageCreateName($file);
                $data->upload($name,$file,$data->gift_banner);
                $input['gift_banner'] = $name;
            }

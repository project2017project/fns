...
class ProductController extends AdminBaseController
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->type == 'Physical') {
            ...
            if (empty($request->size_check)) {
             ...
             // Variation
            if (empty($request->variation_check)) {
                $input['variation'] = null;
            } else {
                $input['variation'] = implode(',', $request->variation);
            }

              // Cross Selling
            if (empty($request->cross_selling_check)) {
                $input['cross_selling'] = null;
            } else {
                $input['cross_selling'] = implode(',', $request->cross_selling);
            }
        ...
        if ($prod->type != 'Physical') {
            if ( $request->slug == null ) {
                $prod->slug = Str::slug($request->slug, '-').'-'.strtolower(Str::random(3).$data->id.Str::random(3));
            }
            else {
                $prod->slug = Str::slug($request->slug, '-');
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($data->type == 'Physical') {
            // --- Validation Section
            $rules = ['sku' => 'min:8|unique:products,sku,'.$id, 'slug' => 'unique:products,slug,'.$id];
            $customs = ['slug.unique' => 'This slug has already been taken.'];
            $validator = \Validator::make($request->all(), $rules, $customs);
        ...
        if ( $request->slug == null ) {
            $data->slug = Str::slug($request->slug, '-').'-'.strtolower($data->sku);
        }
        else {

...
class ProductDetailsController extends FrontBaseController
{
    ...
    public function product(Request $request, $slug)
    {
        ...
        // echo $productId;
        // The product for which you want to find related items

// Retrieve orders where the product with $productId is present in the cart
$relatedItems = DB::table('orders')->get();
// $relatedItems = array();
// foreach ($relatedItems as $cartitem) {
//     $cartproduct = json_decode($cartitem->cart);
//     //print_r($cartproduct->items);
//     foreach($cartproduct->items as $key => $product){
//         $relatedItems[] = $product->item->id;
//     }
// }
...
// Extract related product IDs and quantities from the retrieved orders
foreach ($relatedItems as $order) {
    //print_r(json_decode($order->cart));
    $cartproduct = json_decode($order->cart);
     foreach ($cartproduct->items as $item) {
        //print_r($item);
         if ($item->item->id != $productId) {
             $relatedProductId = $item->item->id;
             $relatedQty = $item->qty;

             // Aggregate related products and their quantities
             if (isset($relatedProducts[$relatedProductId])) {
                 $relatedProducts[$relatedProductId] += $relatedQty;
             } else {
                 $relatedProducts[$relatedProductId] = $relatedQty;
             }
         }
...
// Sort related products by the number of times they were bought together
arsort($relatedProducts);

// Limit the number of related products to retrieve
$limit = 3;
$relatedProducts = array_slice($relatedProducts, 0, $limit, true);

$realatedproductkeys = array_keys($relatedProducts);
$fqproducts = implode(', ', $realatedproductkeys);
...
//print_r($fqproducts);
    ...
    public function checkPincodeAvailablity(Request $request)
    {
        if ($request->ajax() && $request->isMethod('post')) {
            if ($request->filled('valueOfpincode') && $request->filled('typofPayment')) {
                $pincode = $request->get('valueOfpincode');
                $paymentType = $request->get('typofPayment');
                ...
                $response = Http::withHeaders(['Content-Type' => 'application/json'])->post('https://apiv2.shiprocket.in/v1/external/auth/login', ['email' => 'lovekumar123@getnada.com', 'password' => '9472@Techno']);
                if ($response->successful()) {
                    $content = json_decode($response->body(), true);
                    $token = $content['token'];
                    $response2 = Http::withHeaders(['Content-Type' => 'application/json', 'Authorization' => 'Bearer '.$token])->get('https://apiv2.shiprocket.in/v1/external/courier/serviceability/', ['pickup_postcode' => '110030', 'delivery_postcode' => (int) $pincode, 'cod' => $isCode, 'weight' => $weight]);
                    if ($response->successful()) {
                        if ($response2->json()['status'] == 200) {
                            return response()->json(['status' => 'success', 'message' => 'Service Available']);
                        } else {
                            return response()->json(['status' => 'fail', 'message' => 'Service not available']);
                        }
                    }
                } else {
                    return response()->json(['status' => 'fail', 'message' => 'Something went wrong please try again later.']);
                }
            } else {
                return response()->json(['status' => 'fail', 'message' => 'Please enter valid pin code and type of payment']);
            }
        } else {

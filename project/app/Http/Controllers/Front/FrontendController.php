...
class FrontendController extends FrontBaseController
{
    ...
    // CURRENCY SECTION ENDS
        ...
        public function extraIndex()
        {
            ...
            $data['latest_fnscutlery'] = Product::with('user')->whereStatus(1)
            ...
            $data['latest_montavocutlery'] = Product::with('user')->whereStatus(1)
            ...
            $data['latest_tableware'] = Product::with('user')->whereStatus(1)
            ...
            $data['latest_barware'] = Product::with('user')->whereStatus(1)
            ...
            $data['latest_dinnerware'] = Product::with('user')->whereStatus(1)
            ...
            $data['fnscutlery'] = Product::with('user')->whereStatus(1)
            ->where('category_id', '=', 45)
            ->home($this->language->id)
            ->with(['user', 'category'])
            ->orderBy(DB::raw('RAND()'))
            ...
            $data['montavocutlery'] = Product::with('user')->whereStatus(1)
            ->where('category_id', '=', 44)
            ->home($this->language->id)
            ->with(['user', 'category'])
            ->take(8)
            ...
            $data['tableware'] = Product::with('user')->whereStatus(1)
            ->where('category_id', '=', 43)
            ->home($this->language->id)
            ->with(['user', 'category'])
            ->take(8)
            ...
            $data['barware'] = Product::with('user')->whereStatus(1)
            ->where('category_id', '=', 42)
            ->home($this->language->id)
            ->with(['user', 'category'])
            ->take(8)
            ...
            $data['dinnerware'] = Product::with('user')->whereStatus(1)
            ->where('category_id', '=', 40)
            ->home($this->language->id)
            ->with(['user', 'category'])
            ->take(8)
            ...
            $data['popular_products'] = Product::with('user')->whereStatus(1)->whereFeatured(1)
             ...
             $data['popluar_fnscutlery'] = Product::with('user')->whereStatus(1)->whereFeatured(1)
                    ->where('category_id', '=', 45)
                    ->home($this->language->id)
                    ->with(['user', 'category'])
                    ->take(4)
            ...
            $data['popluar_montavocutlery'] = Product::with('user')->whereStatus(1)->whereFeatured(1)
                    ->where('category_id', '=', 44)
                    ->home($this->language->id)
                    ->with(['user', 'category'])
                    ->take(4)
            ...
            $data['popular_tableware'] = Product::with('user')->whereStatus(1)->whereFeatured(1)
                    ->where('category_id', '=', 43)
                    ->home($this->language->id)
                    ->with(['user', 'category'])
                    ->take(4)
            ...
            $data['popular_barware'] = Product::with('user')->whereStatus(1)->whereFeatured(1)
                    ->where('category_id', '=', 42)
                    ->home($this->language->id)
                    ->with(['user', 'category'])
                    ->take(4)
            ...
            $data['popular_dinnerware'] = Product::with('user')->whereStatus(1)->whereFeatured(1)
                    ->where('category_id', '=', 40)
                    ->home($this->language->id)
                    ->with(['user', 'category'])
                    ->take(4)
            ...
            $data['blogs'] = Blog::where('language_id', $this->language->id)->latest()->take(6)->get();
            $data['services'] = DB::table('services')->where('language_id', $this->language->id)->where('user_id','=',0)->take(4)->get();
            $data['reviews'] =  DB::table('reviews')->get();
            $data['deals'] =  DB::table('deals')->get();
            $data['ps'] = $this->ps;
        ...
        public function deals()
        {
            ...
            $data['prods'] = Product::with('user')->whereStatus(1)->whereIsDiscount(1)->whereFeatured(0)
                ->where('discount_date', '>=', date('Y-m-d'))
                ->home($this->language->id)
               ->with(['user', 'category'])
                ->get();
            ...
            $data['prodsf'] = Product::with('user')->whereStatus(1)->whereIsDiscount(1)->whereFeatured(1)
                ->where('discount_date', '>=', date('Y-m-d'))
                ->home($this->language->id)
               ->with(['user', 'category'])
                ->get();

            return view('frontend.deals', $data);
        }
        // -------------------------------- Deals SECTION ENDS----------------------------------------
        ...
        {
            $data['prods'] = Product::with('user')->whereStatus(1)->whereBest(1)
                ->where('discount_date', '>=', date('Y-m-d'))
                ->home($this->language->id)
               ->with(['user', 'category'])
                ->get();

            return view('frontend.gifting', $data);
        }
        // -------------------------------- Deals SECTION ENDS----------------------------------------
        ...
        public function autosearch($slug)
        {
            if (mb_strlen($slug, 'UTF-8') > 1) {
                $search = ' '.$slug;
                $prods = Product::where('name', 'like', '%'.$search.'%')->orWhere('name', 'like', $slug.'%')->orWhere('sku', 'like', '%'.$slug.'%')->where('status', '=', 1)->orderby('id', 'desc')->take(10)->get();

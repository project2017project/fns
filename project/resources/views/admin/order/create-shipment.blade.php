...
@section('content')
    <div class="content-area">
        <div class="mr-breadcrumb">
            <div class="row">
                <div class="col-lg-12">
                    <h4 class="heading">{{ __('Create Shipment') }} <a class="add-btn" href="javascript:history.back();"><i
                    ...
                    <ul class="links">
                        ...
                        <li>
                            <a href="javascript:;">{{ __('Create Shipment') }}</a>
                        </li>
                    </ul>
                </div>
            </div>
        ...
        <div class="order-table-wrap">
            ...
            <div class="row">
                <div class="col-lg-12 mt-4">
                    ...
                    @if(isset($order->shiprocket_shipment_id) && !empty($order->shiprocket_shipment_id))
                        <div class="special-box">
                            <div class="heading-area">
                                <h4 class="title">Shipment Details</h4>
                            </div>
                            <div class="table-responsive-sm">
                                <table class="table">
                                    <tbody>
                                        <tr>
                                            <th width="45%">Shipment Order ID</th>
                                            <th width="10%">:</th>
                                            <td width="45%">{{ $order->shiprocket_order_id }}</td>
                                        </tr>
                                        <tr>
                                            <th width="45%">Shipment ID</th>
                                            <th width="10%">:</th>
                                            <td width="45%">{{ $order->shiprocket_shipment_id }}</td>
                                        </tr>
                                        <tr>
                                            <th width="45%">AWB Code</th>
                                            <th width="10%">:</th>
                                            <td width="45%"><a href="https://shiprocket.co/tracking/{{$order->shiprocket_awb_code}}" target="_blank">{{ $order->shiprocket_awb_code }}</a></td>
                                        </tr>
                                        <tr>
                                            <th width="45%">Shipment Weight</th>
                                            <th width="10%">:</th>
                                            <td width="45%">{{ $order->weight }} Kg.</td>
                                        </tr>
                                        <tr>
                                            <th width="45%">Shipment Dimension [ LxWxH ]</th>
                                            <th width="10%">:</th>
                                            <td width="45%">{{ $order->length.' x '. $order->width.' x '.$order->height }} cm.</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>                                        
                    ...
                    @else
                        <div class="special-box">
                            <div class="heading-area">
                                <h4 class="title">Create Shipment</h4>
                            </div>
                            <div class="table-responsive-sm">
                                <form method="POST" action="{{ route('admin-post-create-shipment', $order->id) }}">
                                    @csrf
                                    <div class="row">
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                <label>Length [ cm ]</label>
                                                <input type="text" name="length" id="length" class="form-control text-right" value="{{$order->length}}" required/>
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                <label>Width [ cm ]</label>
                                                <input type="text" name="width" id="width" class="form-control text-right" value="{{$order->width}}" required/>
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                <label>Height [ cm ]</label>
                                                <input type="text" name="height" id="height" class="form-control text-right" value="{{$order->height}}" required/>
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                <label>Weight [ kg ]</label>
                                                <input type="text" name="weight" id="weight" class="form-control text-right" value="{{$order->weight}}" required/>
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                ...
                                                <select name="courier_id" id="courier_id" class="form-select" required>
                                                    ...
                                                    @if(isset($couriers) && count($couriers)>0)
                                                        @foreach($couriers as $courier)
                                                            <option value="{{$courier->courier_id}}" @if(isset($order->courier_id) && $order->courier_id==$courier->courier_id){{'selected="selected"'}}@endif>{{$courier->name}}</option>     
                                                        @endforeach
                                                    @endif
                                                </select>
                                            </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-3">
                                            <button type="submit" class="btn sendEmail"><i class="fa fa-send"></i> {{ __('Create Shipment') }} </button>
                                        </div>
                                    </div>
                                </form>
                            </div>

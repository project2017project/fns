...
$(document).on('click', '.freq_add', function() {
    var totalButtons = $('.button-fq').length;
    var successCounter = 0;
    var showingProductsDiv = $('#multiple-cart-load');

    showingProductsDiv.addClass('loadernew');  // Add the loader class

    $('.button-fq').each(function(index) {
        var button = $(this);
        setTimeout(function() {
            $.get(button.data('href'), function(data) {
                if (data === 'digital') {
                    ...
                    if (successCounter === totalButtons) {
                        toastr.success(lang.cart_success);
                        showingProductsDiv.removeClass('loadernew');  // Remove the loader class
                    }
                }
            });
        }, index * 300);

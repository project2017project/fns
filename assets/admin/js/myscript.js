...
$(document).on('submit','#geniusformdata-pushnotification',function(e){
    if(confirm('Do you want to send Push Notification') == true){
        e.preventDefault();
        if(admin_loader == 1)   $('.submit-loader').show();
        $('button.addPushNotificationSubmit-btn').prop('disabled',true);
        disablekey();
        $.ajax({
            method:"POST",
            url:$(this).prop('action'),
            ...
            success:function(data) {
                if((data.errors)) {
                    ...
                    $("#modal1 .modal-content .modal-body .alert-danger").focus();
                    $('button.addPushNotificationSubmit-btn').prop('disabled',false);
                    $('#geniusformdata-pushnotification input, #geniusformdata-pushnotification textarea').eq(1).focus();
                }
                else {
                    $('.alert-success').show();
                    $('.alert-success p').html(data);
...
$('#confirm-delete-notification').on('show.bs.modal', function(e) {
  $(this).find('.delete-form').attr('action', $(e.relatedTarget).data('href'));
});
$('#confirm-delete-notification .delete-form').on('submit', function(e) {
  if(admin_loader == 1)   $('.submit-loader').show();
  $.ajax({
      method      :   "POST",
      url         :   $(this).prop('action'),
